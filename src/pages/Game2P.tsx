...
const Game2P = () => {
  ...
  useEffect(() => {
    document.title = `Jogador ${currentTurn} - ${activeTeam} | Campanha 2P`;
  ...
  const [players, setPlayers] = useState<Player[]>(() => getInitialPlayers(activeTeam));

  // When turn changes, reload players for the active team
  useEffect(() => {
    setPlayers(getInitialPlayers(activeTeam));
    setHasActiveInvestment(localStorage.getItem(`investment_${activeTeam}`) === 'true');
    setSelectedPlayerForValue(null);
    setSelectedReserve(null);
    setSelectedStarter(null);
    setShowTransferMarket(false);
  ...
  const handleSellPlayer = (player: Player) => {
    ...
    if (wasStarter) {
      ...
      if (reserveReplacement) {
        updatedPlayers = updatedPlayers.map(p => p.id === reserveReplacement.id ? { ...p, isStarter: true } : p);
  ...
  const handleBuyPlayer = (player: Player, price: number) => {
    const newPlayer: Player = { ...player, id: `bought-${Date.now()}`, isStarter: false };
  ...
  const handlePlay = () => {
    ...
    if (savedPlayers) {
      ...
      if (suspended.length > 0) {
        toast.error(`Jogador(es) suspenso(s): ${suspended.map((p: any) => p.name).join(', ')}. Substitua antes!`);
        return;
      }
    }

    if (currentTurn === 1) {
      // Save P1's match data and switch to P2
      if (savedPlayers) {
        localStorage.setItem('match_players_p1', savedPlayers);
      }
      localStorage.setItem('2p_p1_match_pending', JSON.stringify({
        team: player1Team,
        opponent: opponentName,
      }));
      toast.success(`${player1Team} pronto! Agora é a vez do Jogador 2.`);
      setCurrentTurn(2);
    } else {
      // P2 clicks play → go to P1's match first
      if (savedPlayers) {
        localStorage.setItem('match_players_p2', savedPlayers);
      }
      // Save P2's match info
      localStorage.setItem('2p_p2_match_pending', JSON.stringify({
        team: player2Team,
        opponent: opponentName,
      }));
      // Load P1's players for match
      const p1Players = localStorage.getItem('match_players_p1');
      if (p1Players) {
        localStorage.setItem('match_players', p1Players);
      }
      const p1Match = JSON.parse(localStorage.getItem('2p_p1_match_pending') || '{}');
      // Navigate to P1's match
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center px-4">
          ...
          <button onClick={resetChampionship} className="bg-white hover:bg-gray-100 text-black font-bold text-xl py-4 px-12 rounded-lg transition-colors">
            Avançar
          </button>
        </div>
      </div>
    ...
    <div className="fixed inset-0 overflow-hidden bg-black">
      ...
      <GameMenu
        ...
        onExit={() => {
          ...
          localStorage.removeItem(`players_${player1Team}`);
          localStorage.removeItem(`players_${player2Team}`);
          localStorage.removeItem(`starter_order_${player1Team}`);
          localStorage.removeItem(`starter_order_${player2Team}`);
          localStorage.removeItem(`investment_${player1Team}`);
          localStorage.removeItem(`investment_${player2Team}`);
          ...
          {nextMatch && (
            <div className="container mx-auto px-4 py-8">
              ...
              <MatchCard
                ...
                mode2p={true}
                onPlay2P={handlePlay}
                turn2PLabel={currentTurn === 1 ? "PRONTO" : "JOGAR"}
              />
            </div>
          )}
      ...
      {selectedPlayerForValue && swipe.currentPage === 0 && (
        <PlayerValueModal player={selectedPlayerForValue} onClose={() => setSelectedPlayerForValue(null)} canSell={true} onSell={handleSellPlayer} />
      )}
